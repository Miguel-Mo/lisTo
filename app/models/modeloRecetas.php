...
class modeloRecetas
{
    ...
    {
        $this->db->query('UPDATE receta SET nombre = :nombre, dificultad = :dificultad, tiempoEstimado = :tiempoEstimado WHERE id = :id');


        $this->db->bind(':id', $datos['idRecetaEdit']);
        $this->db->bind(':nombre', $datos['nombreNuevo']);
        $this->db->bind(':dificultad', $datos['dificultad']);
        $this->db->bind(':tiempoEstimado', $datos['tiempoReceta']);


        return $this->db->execute();
    }

    public function eliminarAlimentosReceta($id){
        $this->db->query('DELETE FROM receta_alimento WHERE idReceta=' . $id);
        $this->db->execute();
    }
